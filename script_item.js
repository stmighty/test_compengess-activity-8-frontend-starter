...
const backendIPAddress = "127.0.0.1:3000";        //port of backend
...
const showGroupMembers = async () => {
  ...
  const options = {               // ส่ง request แบบ get
    method: "GET",
    credentials: "include",      // security reason
  };
  await fetch(`http://${backendIPAddress}/items/members`, options)
    .then((response) => response.json())
    .then((data) => {
      const members = data;       //members is array of objects which have student_id and full_name as a key
      members.map((member) => {
        member_list.innerHTML += `    
          <li>${member.full_name}</li>
          `;   // member_list.innerhtml += is change html code (add member)
        // ----------------- FILL IN YOUR CODE UNDER THIS AREA ONLY ----------------- //
        //member_dropdown.innerHTML += `<option value="${member.fullname}">${member.full_name}</option`;
...
// TODO #2.1: Edit group number                                                           //finish
const getGroupNumber = () => {
  return 38;
};

// TODO #2.2: Show group members                                                          //finish
const showGroupMembers = async () => {
  const member_list = document.getElementById("member-list");
  member_list.innerHTML = "";
  const member_dropdown = document.getElementById("name-to-add");
...
const showItemsInTable = (itemsData) => {
  ...
  table_body.innerHTML = "";
  // ----------------- FILL IN YOUR CODE UNDER THIS AREA ONLY ----------------- //
  itemsData.sort((a,b) => {                     //this is how to sort javascript object //https://www.javascripttutorial.net/array/javascript-sort-an-array-of-objects/
    return a.created_date - b.created_date;
  })
  // ----------------- FILL IN YOUR CODE ABOVE THIS AREA ONLY ----------------- //
  itemsData.map((item) => {    //.map ก็เหมือน for loop แต่ละรอบ
    // ----------------- FILL IN YOUR CODE UNDER THIS AREA ONLY ----------------- //      //specify row with item_id
    table_body.innerHTML += `
        <tr id="${item.item_id}">      
            <td>${item.item}</td>
            <td>${item.name}</td>
...
const addItem = async () => {
  ...
  //create object that has 3 fields(item, name, price)
  const itemToAdd = {
    item: item,
    name: name,
    price: price
  ...
  //we need to clear value of each variable
  await fetch(`http://${backendIPAddress}/items`, options)
    .then((response) =>
    {
      document.getElementById("item-to-add").value = "";
